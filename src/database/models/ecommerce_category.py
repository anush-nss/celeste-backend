...
class EcommerceCategory(Base):
    ...
    parent_category_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("ecommerce_categories.id", ondelete="CASCADE"), nullable=True, index=True)
    ...
    subcategories: Mapped[List["EcommerceCategory"]] = relationship(
        "EcommerceCategory", back_populates="parent_category", cascade="all, delete-orphan"
    )