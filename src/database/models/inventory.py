...
class Inventory(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "quantity_available >= 0", name="check_quantity_available_non_negative"
        ),
        CheckConstraint(
            "quantity_reserved >= 0", name="check_quantity_reserved_non_negative"
        ),
        CheckConstraint(
        ...
        Index(
            "idx_inventory_reserved",
            "store_id",
            "quantity_reserved",
            postgresql_where="quantity_reserved > 0",
        ),
        Index(
            "idx_inventory_on_hold",
            "store_id",
            "quantity_on_hold",
    ...
    product: Mapped["Product"] = relationship(
        "Product", back_populates="inventory_levels"
    )
