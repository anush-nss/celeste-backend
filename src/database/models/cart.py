...
class Cart(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_carts_user_active', 'created_by', 'status',
              postgresql_where="status = 'active'"),

        # Cart lifecycle tracking
        Index('idx_carts_created_at', 'created_at'),
        Index('idx_carts_updated_at', 'updated_at'),
        Index('idx_carts_ordered_at', 'ordered_at',
              postgresql_where="ordered_at IS NOT NULL"),

        # Status-based filtering
        Index('idx_carts_active', 'status', 'created_by',
              postgresql_where="status = 'active'"),
        Index('idx_carts_ordered', 'status', 'ordered_at',
              postgresql_where="status = 'ordered'"),

        # Cart analytics and cleanup
        Index('idx_carts_stale', 'updated_at', 'status',
              postgresql_where="status = 'active'"),

        # Search and management
        Index('idx_carts_name_trgm', 'name', postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'}),

        # Composite indexes for cart management
        Index('idx_carts_user_lifecycle', 'created_by', 'status', 'created_at'),
        Index('idx_carts_conversion_tracking', 'created_by', 'created_at', 'ordered_at'),
    )
...
class CartUser(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_cart_users_owners', 'cart_id', 'user_id',
              postgresql_where="role = 'owner'"),
        Index('idx_cart_users_viewers', 'cart_id', 'user_id',
...
class CartItem(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_cart_items_product_analytics', 'product_id', 'quantity', 'created_at'),

        # Recent activity tracking
        Index('idx_cart_items_recent', 'updated_at', 'cart_id'),

        # Comprehensive cart item analysis
        Index('idx_cart_items_full_analysis', 'cart_id', 'product_id', 'quantity', 'created_at', 'updated_at'),
    )
