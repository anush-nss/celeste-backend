...
class InventoryService:
    ...
    async def get_all_inventory(self, product_id: Optional[str] = None, store_id: Optional[str] = None) -> list[InventorySchema]:
    ...
    async def create_inventory(self, inventory_data: CreateInventorySchema) -> InventorySchema:
    ...
    async def update_inventory(self, inventory_id: str, inventory_data: UpdateInventorySchema) -> InventorySchema | None:
        doc_ref = self.inventory_collection.document(inventory_id)
        if not doc_ref.get().exists:
            return None
        inventory_dict = inventory_data.model_dump(exclude_unset=True)
        if 'productId' in inventory_dict: del inventory_dict['productId']
        if 'storeId' in inventory_dict: del inventory_dict['storeId']
        if 'stock' in inventory_dict: del inventory_dict['stock']
