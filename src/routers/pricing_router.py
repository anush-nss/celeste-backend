...
async def delete_price_list_line(line_id: str):
    ...
    if not success:
        raise ResourceNotFoundException(detail=f"Price list line with ID {line_id} not found")
    return success_response({"id": line_id, "message": "Price list line deleted successfully"})

# Price Calculation Endpoints (Public/Customer)
@pricing_router.post("/calculate-price", summary="Calculate price for a single product", response_model=PriceCalculationResponse)
async def calculate_price(request: PriceCalculationRequest):
    ...
    if request.customer_tier:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid customer tier: {request.customer_tier}")
