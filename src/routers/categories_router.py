...
from src.models.category_models import CategorySchema, CreateCategorySchema, UpdateCategorySchema
...
@categories_router.get("/", summary="Get all categories", response_model=List[CategorySchema])
async def get_all_categories():
    categories = await category_service.get_all_categories()
    return success_response([c.model_dump(mode='json') for c in categories])

@categories_router.get("/{id}", summary="Get a category by ID", response_model=CategorySchema)
...
@categories_router.put("/{id}", summary="Update a category", response_model=CategorySchema, dependencies=[Depends(RoleChecker([UserRole.ADMIN]))])
...
@categories_router.delete("/{id}", summary="Delete a category", dependencies=[Depends(RoleChecker([UserRole.ADMIN]))])
