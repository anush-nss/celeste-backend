...
class TierRequirementsSchema(BaseModel):
    min_orders: int = Field(default=0, ge=0, description="Minimum number of orders required")
    min_lifetime_value: float = Field(default=0.0, ge=0, description="Minimum lifetime value required")
    min_monthly_orders: int = Field(default=0, ge=0, description="Minimum monthly orders required")

class TierBenefitsSchema(BaseModel):
    price_list_ids: List[str] = Field(default_factory=list, description="Price list IDs available to this tier")
    delivery_discount: float = Field(default=0.0, ge=0, le=100, description="Delivery discount percentage")
    priority_support: bool = Field(default=False, description="Whether tier gets priority support")
    early_access: bool = Field(default=False, description="Whether tier gets early access to products")

class CustomerTierSchema(BaseModel):
    id: Optional[str] = None
    name: str = Field(..., min_length=1, description="Tier name (e.g., 'Gold', 'Platinum')")
    tier_code: CustomerTier = Field(..., description="Tier code")
    level: int = Field(..., ge=1, description="Tier level (higher number = better tier)")
    requirements: TierRequirementsSchema = Field(..., description="Requirements to achieve this tier")
...
class UpdateCustomerTierSchema(BaseModel):
    ...
    requirements: Optional[TierRequirementsSchema] = Field(None, description="Requirements to achieve this tier")
    benefits: Optional[TierBenefitsSchema] = Field(None, description="Benefits of this tier")
    icon_url: Optional[str] = Field(None, description="URL to tier icon")
