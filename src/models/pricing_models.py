...
from src.shared.constants import PriceListType, DiscountType

class PriceListSchema(BaseModel):
    id: Optional[str] = None
    name: str = Field(..., min_length=1, description="Price list name (e.g., 'VIP Discount', 'Bulk Pricing')")
    ...
    valid_until: Optional[datetime] = Field(None, description="When this price list expires (null = never expires)")
...
class UpdatePriceListSchema(BaseModel):
    ...
    active: Optional[bool] = Field(None, description="Whether this price list is active")
    valid_from: Optional[datetime] = Field(None, description="When this price list becomes valid")
    valid_until: Optional[datetime] = Field(None, description="When this price list expires")
...
class PriceListLineSchema(BaseModel):
    ...
    product_id: Optional[str] = Field(None, description="Product ID (required if type='product')")
    category_id: Optional[str] = Field(None, description="Category ID (required if type='category')")
    discount_type: DiscountType = Field(..., description="Discount type")
    amount: float = Field(..., ge=0, description="Discount amount (percentage or flat amount)")
    min_product_qty: int = Field(default=1, ge=1, description="Minimum quantity required")
    max_product_qty: Optional[int] = Field(None, ge=1, description="Maximum quantity allowed (null = no limit)")
    created_at: Optional[datetime] = None
    updated_at: Optional[datetime] = None
...
class CreatePriceListLineSchema(BaseModel):
    ...
    min_product_qty: int = Field(default=1, ge=1, description="Minimum quantity required")
    max_product_qty: Optional[int] = Field(None, ge=1, description="Maximum quantity allowed")
    ...
    def validate_type_fields(self):
        ...
        if self.type == PriceListType.ALL and (self.product_id or self.category_id):
            raise ValueError("product_id and category_id must be null when type is 'all'")
...
class UpdatePriceListLineSchema(BaseModel):
    ...
    min_product_qty: Optional[int] = Field(None, ge=1, description="Minimum quantity required")
    max_product_qty: Optional[int] = Field(None, ge=1, description="Maximum quantity allowed")
...
class PriceCalculationResponse(BaseModel):
    ...
    quantity: int
    applied_price_lists: List[str] = Field(default_factory=list, description="Names of applied price lists")

class BulkPriceCalculationRequest(BaseModel):
    items: List[PriceCalculationRequest] = Field(..., description="List of products to calculate prices for")
