...
class TiersCache:
    ...
    def invalidate_tier_cache(self, tier_id: Optional[str] = None, tier_code: Optional[str] = None) -> int:
        ...
        if tier_id:
            deleted += cache_invalidation_manager.invalidate_cross_domain_dependencies('tiers', tier_id)
        
        if deleted > 0:
            logger.info(f"Invalidated {deleted} tiers cache keys for tier: {tier_id or tier_code or 'all'}")
