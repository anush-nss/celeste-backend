...
class ProductsCache:
    ...
    def invalidate_product_cache(self, product_id: Optional[str] = None) -> int:
        ...
        if product_id:
            ...
            deleted += cache_invalidation_manager.invalidate_cross_domain_dependencies('products', product_id)
        else:
            # Invalidate all products and related pricing
            deleted += self.cache.delete_pattern(f"{self.prefix}:*")
            deleted += cache_invalidation_manager.invalidate_cross_domain_dependencies('products')
        
        if deleted > 0:
            logger.info(f"Invalidated {deleted} products cache keys for product: {product_id or 'all'}")
