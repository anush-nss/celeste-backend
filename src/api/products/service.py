    ...
    async def get_all_products(
        ...
        if query_params.categoryId:
            products_ref = products_ref.where(
                "categoryId", "==", query_params.categoryId
            )
        ...
        if query_params.isFeatured is not None:
            products_ref = products_ref.where(
                "isFeatured", "==", query_params.isFeatured
            )
    ...
    async def get_products_with_pagination(
        ...
        for doc in docs:
            ...
            if doc_dict:
                ...
                if (
                    query_params.minPrice is not None
                    and doc_dict.get("price", 0) < query_params.minPrice
                ):
                    continue
                if (
                    query_params.maxPrice is not None
                    and doc_dict.get("price", 0) > query_params.maxPrice
                ):
                    continue
                ...
                if (
                    primary_filter != "isFeatured"
                    and query_params.isFeatured is not None
                ):
                    if doc_dict.get("isFeatured", False) != query_params.isFeatured:
                        continue

                products.append({"id": doc.id, "data": doc_dict, "doc_ref": doc})
        ...
        if query_params.include_pricing and pricing_service and products:
            ...
            for prod in products:
                enhanced_product = EnhancedProductSchema(id=prod["id"], **prod["data"])
                enhanced_products.append(enhanced_product)
        ...
        return PaginatedProductsResponse(
            products=enhanced_products, pagination=pagination
        )
