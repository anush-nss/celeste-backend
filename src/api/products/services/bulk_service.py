        ...
        async with AsyncSessionLocal() as session:
                ...
                if p.ref and p.ref.strip():
                    ...
                    if ref_counts[ref] > 1:
                        raise ValidationException(
                            detail=f"Duplicate ref '{ref}' found in request"
                        )
            ...
            for p in products_data:
                if p.id:
                    ...
                    if id_counts[p.id] > 1:
                        raise ValidationException(
                            detail=f"Duplicate ID {p.id} found in request"
                        )
            ...
            existing_products = (
                (
                    await session.execute(
                        select(Product).filter(Product.name.in_(product_names))
                    )
                )
                .scalars()
                .all()
            ...
            if existing_products:
                raise ConflictException(
                    f"Products with these names already exist: {[p.name for p in existing_products]}"
                )
            ...
            if product_refs:
                existing_refs = (
                    (
                        await session.execute(
                            select(Product).filter(Product.ref.in_(product_refs))
                        )
                    )
                    .scalars()
                    .all()
                ...
                if existing_refs:
                    raise ConflictException(
                        f"Products with these refs already exist: {[p.ref for p in existing_refs]}"
                    )
            ...
            if product_ids:
                ...
                if existing_ids:
                    raise ConflictException(
                        f"Products with these IDs already exist: {[p.id for p in existing_ids]}"
                    )
            ...
            if all_category_ids:
                categories = (
                    (
                        await session.execute(
                            select(Category).filter(Category.id.in_(all_category_ids))
                        )
                    )
                    .scalars()
                    .all()
            ...
            if all_tag_ids:
                tags = (
                    (await session.execute(select(Tag).filter(Tag.id.in_(all_tag_ids))))
                    .scalars()
                    .all()
                )
            ...
            for i, product in enumerate(new_products):
                ...
                if data.category_ids:
                    for cid in data.category_ids:
                        product_category_associations.append(
                            {"product_id": product.id, "category_id": cid}
                        )
                if data.tag_ids:
                    for tid in data.tag_ids:
                        product_tag_associations.append(
                            {
            ...
            if product_category_associations:
                ...
                await session.execute(
                    product_categories.insert(), product_category_associations
                )

            if product_tag_associations:
                await session.execute(
                    ProductTag.__table__.insert(), product_tag_associations
                )
            ...
            pydantic_products = safe_model_validate_list(
                ProductSchema, final_products, max_depth=3
            )
