...
class StoreSelectionService:
    ...
    async def select_stores_for_delivery(
        ...
        try:
            ...
            address_coords = await self._get_address_coordinates(address_id)
            ...
            nearby_stores = await self._get_stores_by_distance(address_coords)
            ...
            availability = await self._check_store_availability(nearest_store["store_id"], cart_items)
            ...
            if availability["all_available"]:
                ...
                split_result = await self._split_items_across_stores(cart_items, nearby_stores)
                selection_result.update({
                    "primary_store": nearest_store,
    ...
    async def validate_pickup_store(
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                availability = await self._check_store_availability(store_id, cart_items)
    ...
    async def _get_address_coordinates(self, address_id: int) -> Optional[Tuple[float, float]]:
        """Get address coordinates from database"""

        # TODO: Implement actual address lookup
        # Placeholder - return dummy coordinates
        # In real implementation, query addresses table
        return (40.7128, -74.0060)  # Example NYC coordinates
    ...
    async def _split_items_across_stores(
        ...
        for store in nearby_stores:
            ...
            availability = await self._check_store_availability(store_id, remaining_items)
