...
class CategoriesCache:
    ...
    def invalidate_category_cache(self, category_id: Optional[str] = None) -> int:
        ...
        if category_id:
            deleted += cache_invalidation_manager.invalidate_cross_domain_dependencies('categories', category_id)
        
        if deleted > 0:
            logger.info(f"Invalidated {deleted} categories cache keys for category: {category_id or 'all'}")
