...
class PricingCache:
    ...
    def get_product_pricing_key(self, product_id: str, base_price: float, 
                               category_id: Optional[str], customer_tier: str, 
                               quantity: int = 1) -> str:
        """Generate cache key for individual product pricing"""
        return self.cache.generate_key(
            f"{self.prefix}_product", product_id, base_price, 
            category_id or "none", customer_tier, quantity
        )
    ...
    def get_bulk_pricing_key(self, customer_tier: str, product_ids: List[str]) -> str:
        ...
        return self.cache.generate_key(f"{self.prefix}_bulk", customer_tier, product_hash)
    
    def get_price_lists_key(self, active_only: bool = False) -> str:
        """Generate cache key for price lists"""
        return self.cache.generate_key(f"{self.prefix}_lists", "active" if active_only else "all")
    ...
    def get_product_pricing(self, product_id: str, base_price: float, 
                           category_id: Optional[str], customer_tier: str, 
                           quantity: int = 1) -> Optional[Dict]:
        """Get cached product pricing"""
        key = self.get_product_pricing_key(product_id, base_price, category_id, customer_tier, quantity)
        return self.cache.get(key)
    
    def set_product_pricing(self, product_id: str, base_price: float, 
                           category_id: Optional[str], customer_tier: str, 
                           pricing_data: Dict, quantity: int = 1) -> bool:
    ...
    def get_bulk_pricing(self, customer_tier: str, product_ids: List[str]) -> Optional[List[Dict]]:
        """Get cached bulk pricing"""
        key = self.get_bulk_pricing_key(customer_tier, product_ids)
        return self.cache.get(key)
    
    def set_bulk_pricing(self, customer_tier: str, product_ids: List[str], 
                        pricing_data: List[Dict]) -> bool:
    ...
    def invalidate_pricing_cache(self, scope: str = "pricing") -> int:
        ...
        patterns = {
            "all": [f"{self.prefix}_product:*", f"{self.prefix}_bulk:*", 
                   f"{self.prefix}_lists:*", f"{self.prefix}_lines:*"],
    ...
    def invalidate_price_list_cache(self, price_list_id: Optional[str] = None) -> int:
        ...
        if deleted > 0:
            logger.info(f"Invalidated {deleted} pricing cache keys for price list: {price_list_id or 'all'}")
