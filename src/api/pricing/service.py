...
class PricingService:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if existing.scalars().first():
                raise ConflictException(
                    detail=f"Price list with name '{price_list_data.name}' already exists"
                )

            new_price_list = PriceList(
                name=price_list_data.name.strip(),
                description=price_list_data.description.strip()
                if price_list_data.description
                else None,
            ...
            # Invalidate cache when price list is created
    ...
    async def get_price_list_by_id(
        ...
        async with AsyncSessionLocal() as session:
            result = await session.execute(
                select(PriceList)
                .options(selectinload(PriceList.lines))
    ...
    async def get_all_price_lists(
        ...
        async with AsyncSessionLocal() as session:
            ...
            if active_only:
                query = query.where(PriceList.is_active)
    ...
    async def update_price_list(
        self, price_list_id: int, price_list_data: UpdatePriceListSchema
    ) -> Optional[PriceListSchema]:
        """Update a price list"""
        async with AsyncSessionLocal() as session:
            result = await session.execute(
            ...
            # Invalidate cache when price list is updated
    ...
    async def create_price_lists(
        self, price_lists_data: list[CreatePriceListSchema]
    ) -> list[PriceListSchema]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            for price_list_data in price_lists_data:
                ...
                # Check for duplicate name
                existing = await session.execute(
                    select(PriceList).filter(
                        PriceList.name == price_list_data.name.strip()
                    )
                )
                if existing.scalars().first():
                    raise ConflictException(
            ...
            # Invalidate cache when price lists are created
            pricing_cache.invalidate_price_list_cache()
    ...
    async def add_price_list_line(
        self, price_list_id: int, line_data: CreatePriceListLineSchema
    ) -> PriceListLineSchema:
        ...
        async with AsyncSessionLocal() as session:
            ...
            # Invalidate cache when price list line is added
            pricing_cache.invalidate_price_list_cache(str(price_list_id))
    ...
    async def update_price_list_line(
        self, line_id: int, line_data: UpdatePriceListLineSchema
    ) -> Optional[PriceListLineSchema]:
        """Update a price list line"""
        async with AsyncSessionLocal() as session:
            result = await session.execute(
                select(PriceListLine).where(PriceListLine.id == line_id)
            )
            ...
            # Invalidate cache when price list line is updated
            pricing_cache.invalidate_price_list_cache(str(line.price_list_id))
    ...
    async def add_price_list_lines(
        self, price_list_id: int, lines_data: list[CreatePriceListLineSchema]
    ) -> list[PriceListLineSchema]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            # Invalidate cache when price list lines are added
            pricing_cache.invalidate_price_list_cache(str(price_list_id))
            ...
            try:
                ...
                existing = await session.execute(
                    select(TierPriceList).where(
                        and_(
                            TierPriceList.tier_id == tier_id,
                            TierPriceList.price_list_id == price_list_id,
                        )
                    )
                )
                if existing.scalars().first():
                    ...
                    raise ConflictException(
                        detail=f"Price list {price_list_id} is already assigned to tier {tier_id}"
                    )

                association = TierPriceList(
                    tier_id=tier_id, price_list_id=price_list_id
                )
                ...
                if "tier_id" in error_detail and "is not present" in error_detail:
                    ...
                    "price_list_id" in error_detail and "is not present" in error_detail
                ):
                    raise ResourceNotFoundException(
                        detail=f"Price list with ID {price_list_id} not found"
                    )
    ...
    async def remove_price_list_from_tier(
        self, tier_id: int, price_list_id: int
    ) -> bool:
    ...
    async def calculate_product_price(
        ...
        async with AsyncSessionLocal() as session:
            ...
            if user_tier_id:
                ...
                active_price_lists = [
                    tpl.price_list
                    for tpl in tier_price_list_associations
                    if tpl.price_list and tpl.price_list.is_active
                ]
                ...
                if price_list_ids:
                    ...
                    valid_price_lists = [
                        ...
                        if (not pl.valid_from or now >= pl.valid_from)
                        and (not pl.valid_until or now <= pl.valid_until)
                    ]
                    ...
                    if valid_price_lists:
                        ...
                        lines_query = (
                            ...
                            .where(
                                PriceListLine.price_list_id.in_(
                                    [pl.id for pl in valid_price_lists]
                                )
                            )
                            .where(PriceListLine.is_active)
                        ...
                        line_conditions = [
                            ...
                            PriceListLine.category_id.in_(product_category_ids)
                            if product_category_ids
                            else None,
                            and_(
                                PriceListLine.product_id.is_(None),
                                PriceListLine.category_id.is_(None),
                            ),
                        ]
                        # Filter out None conditions
                        line_conditions = [
                            cond for cond in line_conditions if cond is not None
                        ]
                        ...
                        for price_list in valid_price_lists:
                            if price_list.id in lines_by_price_list:
                                ...
                                for line in lines:
                                    discount_applied = self._apply_discount(
                                        base_price, line, quantity
                                    )
                                        ...
                                        applied_discounts.append(
                                            {
                                                ...
                                                "discounted_price": float(
                                                    round(discount_applied, 2)
                                                ),
                                                "savings": float(
                                                    round(
                                                        base_price - discount_applied, 2
                                                    )
                                                ),
                                            }
                                        )
    ...
    def _apply_discount(
        self, base_price: Decimal, line: PriceListLine, quantity: int
    ) -> Decimal:
    ...
    async def calculate_bulk_product_pricing(
        self, product_data: List[Dict[str, Any]], user_tier_id: Optional[int]
    ) -> List[ProductPricingSchema]:
        ...
        # Check cache first
        cached_pricing = pricing_cache.get_bulk_pricing(
            str(user_tier_id), [str(pid) for pid in product_ids]
        )
        if cached_pricing:
        ...
        async with AsyncSessionLocal() as session:
            ...
            pricing_result = await session.execute(
                text(pricing_query),
                {"tier_id": user_tier_id, "product_ids": product_ids},
            )
            ...
            for p_data in product_data:
                ...
                if product_id in pricing_dict:
                    ...
                    applied_discounts = (
                        [
                            {
                                ...
                                "discount_value": float(pricing_row.discount_value)
                                if pricing_row.discount_value
                                else 0.0,
                        ...
                        if pricing_row.price_list_name
                        else []
                    )
                else:
                    final_price = base_price
            ...
            # Cache the results
            pricing_cache.set_bulk_pricing(
                str(user_tier_id),
    ...
    async def _price_list_line_to_schema(
        self, line: PriceListLine
    ) -> PriceListLineSchema:
