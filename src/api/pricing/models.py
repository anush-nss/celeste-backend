...
class PriceListSchema(BaseModel):
    ...
    priority: int = Field(
        default=0, description="Priority order (higher = more priority)"
    )
    ...
    is_active: bool = Field(
        default=True, description="Whether this price list is active"
    )
...
class PriceListLineSchema(BaseModel):
    ...
    product_id: Optional[int] = Field(
        None, description="Specific product (null = all products)"
    )
    category_id: Optional[int] = Field(
        None, description="Product category (null = all categories)"
    )
    ...
    max_discount_amount: Optional[float] = Field(
        None, description="Maximum discount cap for percentage discounts"
    )
