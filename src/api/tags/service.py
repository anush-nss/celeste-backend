...
class TagService:
    ...
    async def create_tag(self, tag_data: CreateTagSchema) -> Tag:
        ...
        async with AsyncSessionLocal() as session:
            ...
            slug = tag_data.slug or self._generate_slug(
                tag_data.name, tag_data.tag_type
            )

            # Ensure slug is unique
            existing_tag = await session.execute(select(Tag).filter(Tag.slug == slug))
            ...
            tag_type = (
                f"{self.entity_type}_{tag_data.tag_type}"
            ...
            try:
                ...
                raise ConflictException(
                    detail="Tag creation failed due to constraint violation"
                )
    ...
    async def get_tags_by_type(
        self, is_active: Optional[bool] = True, tag_type_suffix: Optional[str] = None
    ) -> List[Tag]:
    ...
    async def get_tag_by_id(self, tag_id: int) -> Optional[Tag]:
        ...
        async with AsyncSessionLocal() as session:
            result = await session.execute(
                select(Tag).filter(
                    Tag.id == tag_id, Tag.tag_type.like(f"{self.entity_type}_%")
                )
            )
    ...
    async def get_tag_by_slug(self, slug: str) -> Optional[Tag]:
        ...
        async with AsyncSessionLocal() as session:
            result = await session.execute(
                select(Tag).filter(
                    Tag.slug == slug, Tag.tag_type.like(f"{self.entity_type}_%")
                )
            )
            return result.scalars().first()
    ...
    async def update_tag(self, tag_id: int, tag_data: UpdateTagSchema) -> Optional[Tag]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if "tag_type" in update_dict:
                update_dict["tag_type"] = (
                    f"{self.entity_type}_{update_dict['tag_type']}"
                )
            ...
            try:
                ...
                await session.rollback()
                raise ConflictException(
                    detail="Tag update failed due to constraint violation"
                )
    ...
    async def assign_tag_to_entity(
        self, entity_id: int, tag_id: int, value: Optional[str] = None
    ):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                if (
                    f"{self.entity_type}_id" in error_detail
                    and "is not present" in error_detail
                ):
                    raise ResourceNotFoundException(
                        detail=f"{self.entity_type.capitalize()} with ID {entity_id} not found"
                    )
                    ...
                    raise ConflictException(
                        detail=f"{self.entity_type.capitalize()} {entity_id} already has tag {tag_id}"
                    )
                else:
                    raise
                ...
                if self.entity_type == "product":
                    ...
                    result = await session.execute(
                        select(StoreTag).filter(
                            StoreTag.store_id == entity_id, StoreTag.tag_id == tag_id
                        )
                    )
                    entity_tag = result.scalars().first()
                else:
    ...
    def _parse_single_tag_filter(
        self, filter_str: str, param_offset: int
    ...
    def build_tag_filter_query_conditions(
        self, base_conditions: List[str], tag_filter_result: Dict[str, Any]
