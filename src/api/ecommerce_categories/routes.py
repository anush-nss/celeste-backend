...
ecommerce_categories_router = APIRouter(
    prefix="/ecommerce-categories", tags=["Ecommerce Categories"]
)
...
async def get_all_ecommerce_categories(
    include_subcategories: bool = Query(
        True, description="Whether to include subcategories in the response"
    ),
):
    categories = await ecommerce_category_service.get_all_categories(
...
async def get_ecommerce_category_by_id(id: int):
    ...
    if not category:
        raise ResourceNotFoundException(
            detail=f"Ecommerce category with ID {id} not found"
        )
...
async def create_ecommerce_categories(
    payload: Union[CreateEcommerceCategorySchema, List[CreateEcommerceCategorySchema]],
):
    ...
    created_categories = await ecommerce_category_service.create_categories(
...
async def update_ecommerce_category(
    id: int, category_data: UpdateEcommerceCategorySchema
):
    updated_category = await ecommerce_category_service.update_category(
        id, category_data
    )
...
async def delete_ecommerce_category(id: int):
    ...
    return success_response(
        {"id": id, "message": "Ecommerce category deleted successfully"}
    )
