...
class CacheInvalidationManager:
    ...
    def invalidate_pricing_dependencies(
        ...
        logger.debug(
            f"Invalidating pricing caches due to {changed_domain} change: {changed_id or 'all'}"
        )
    ...
    def invalidate_cross_domain_dependencies(
        ...
        for affected_domain in affected_domains:
            if affected_domain == "pricing":
                total_deleted += self.invalidate_pricing_dependencies(
                    changed_domain, changed_id
                )
