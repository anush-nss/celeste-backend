...
from src.config.constants import CacheScopes, Collections
...
class CacheInvalidationManager:
    ...
    def __init__(self):
        ...
        self._dependency_map: Dict[str, List[str]] = {
            ...
            Collections.PRICE_LISTS: [
                Collections.PRICE_LISTS
            ],  # Price list changes affect pricing cache
    ...
    def invalidate_entity(
        ...
        if scope in [
            CacheInvalidationScope.CROSS_DOMAIN,
            CacheInvalidationScope.GLOBAL,
        ]:
        ...
        logger.info(
            f"Cache invalidation completed. Total keys deleted: {total_deleted}"
        )
        return total_deleted

    def _invalidate_domain_cache(
        self, domain: str, entity_id: Optional[str] = None
    ) -> int:
        ...
        if domain == Collections.PRODUCTS:
            ...
            if entity_id:
                ...
                deleted = cache_instance.invalidate_pricing_cache(
    ...
    def _execute_invalidation_hooks(
        self, domain: str, entity_id: Optional[str] = None
    ) -> int:
    ...
    def _invalidate_dependencies(
        self, changed_domain: str, entity_id: Optional[str] = None
    ) -> int:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        for domain, cache_instance in self._domain_caches.items():
            if hasattr(cache_instance, "cache") and hasattr(
                cache_instance.cache, "get_cache_stats"
            ):
    ...
    def invalidate_pricing_dependencies(
        ...
        logger.warning(
            "invalidate_pricing_dependencies is deprecated, use invalidate_entity instead"
        )
    ...
    def invalidate_cross_domain_dependencies(
        ...
        """Legacy method - use invalidate_entity instead"""
        logger.warning(
            "invalidate_cross_domain_dependencies is deprecated, use invalidate_entity instead"
        )
        return self._invalidate_dependencies(changed_domain, changed_id)
