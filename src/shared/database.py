...
class AsyncDatabaseClient:
    ...
    def __init__(self):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=20, thread_name_prefix="firestore_pool"
        )
    ...
    async def get_async_collection(self, collection_name: str):
        ...
        if collection_name not in self._async_collections:
            ...
            self._async_collections[collection_name] = client.collection(
                collection_name
            )
    ...
    def get_sync_collection(self, collection_name: str):
        ...
        if collection_name not in self._collections:
            self._collections[collection_name] = self.get_sync_client().collection(
                collection_name
            )
