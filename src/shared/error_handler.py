...
class ErrorHandler:
    ...
    def handle_database_error(
        ...
        if isinstance(error, IntegrityError):
            ...
            self.logger.error(
                f"Database integrity error during {operation}: {error_msg}",
                extra=context,
            )
            ...
            if "foreign key constraint" in error_msg.lower():
                raise ResourceNotFoundException(
                    detail=f"Referenced resource not found for {operation}"
                )
            elif (
                ...
                raise ConflictException(
                    detail=f"Resource already exists for {operation}"
                )
            else:
                raise ServiceError(
                    f"Data integrity error during {operation}", error, context
                )

        elif isinstance(error, DatabaseError):
            self.logger.error(
                f"Database error during {operation}: {str(error)}", extra=context
            )
            raise ServiceError(
                f"Database operation failed for {operation}", error, context
            )

        elif isinstance(error, SQLAlchemyError):
            self.logger.error(
                f"SQLAlchemy error during {operation}: {str(error)}", extra=context
            )
            raise ServiceError(
                f"Database operation failed for {operation}", error, context
            )
    ...
    def handle_firebase_error(
        ...
        if "UserNotFoundError" in error_type:
            self.logger.warning(
                f"Firebase user not found during {operation}", extra=context
            )
            ...
            self.logger.warning(
                f"Invalid Firebase token during {operation}", extra=context
            )
            ...
            self.logger.warning(
                f"Expired Firebase token during {operation}", extra=context
            )
            ...
            self.logger.warning(
                f"Revoked Firebase token during {operation}", extra=context
            )
            raise UnauthorizedException(detail="Authentication token revoked")

        else:
            self.logger.error(
                f"Firebase error during {operation}: {str(error)}", extra=context
            )
            raise ServiceError(
                f"Authentication service error during {operation}", error, context
            )
    ...
    def handle_general_error(
        ...
        if isinstance(
            ...
            self.logger.info(
                f"Known error during {operation}: {error.detail}", extra=context
            )
            raise error
            ...
            self.logger.error(
                f"Service error during {operation}: {error.message}", extra=context
            )
        ...
        async def async_wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                if isinstance(e, HTTPException):
                    ...
                    "firebase" in str(type(e)).lower() or "auth" in str(type(e)).lower()
                ):
